package  educloud.cloudserver.nodecllient;
...
public class VMNodeClient extends AbstractNodeClient {
	...
	public void changeVM(VirtualMachine machine) throws NodeComunicationException {
		...
		try {
			response = getResouce().path("vm").path("change").accept(MediaType.APPLICATION_JSON).put(ClientResponse.class, jsonMachine);
		} catch (ClientHandlerException e) {
			throw new NodeComunicationException("Error on stop the virtual machine from node", e);
		}
	...
	public void createVM(VirtualMachine vm, Template template) throws NodeComunicationException {
		...
		try {
			...
			throw new NodeComunicationException("Error on create the virtual machine from node", e);
		}

		if (response.getStatus() != 200) {
			throw new NodeComunicationException("Error on create the virtual machine from node");
		}
