...
import  educloud.api.entities.RDPConfig;
import  educloud.api.entities.Template;
import  educloud.api.entities.VirtualMachine;
import  educloud.api.entities.VirtualMachine.VMState;
import  educloud.api.to.NewVirtualMachineTO;
...
import  educloud.cloudserver.entity.CloudSession;
import  educloud.cloudserver.entity.User;
import  educloud.cloudserver.entity.User.UserType;
import  educloud.cloudserver.managers.VMManager;
...
public class VMRest extends CloudResource {
	...
	public Response createVM(String machine) {
		...
		//Cria uma entidade interna (Virtual Machine) baseada na recebida.
		 educloud.internal.entities.VirtualMachine vm =
			new  educloud.internal.entities.VirtualMachine();

		//Cria uma entidade interna (Template) baseada na recebida
		 educloud.internal.entities.Template tpt = null;
		...
		vm.setState( educloud.internal.entities.VirtualMachine.VMState.DONE);
		vm.setMemorySize(tpt.getMemorySize());
		vm.setNumberProcessors(tpt.getNumberProcessors());
		vm.setWithLoadBalancer(externalMachine.isWithLoadBalancer());
		...
		externalMachine.setNumberProcessors(tpt.getNumberProcessors());
	...
	public Response startVM(String machine) {
		...
		LOG.debug("Application will start a new VM (Server Side)");
		LOG.debug(machine);
		...
		/* create internal entity (Virtual Machine) based on received */
		 educloud.internal.entities.VirtualMachine vm = new  educloud.internal.entities.VirtualMachine();
		...
		if (vm == null) {
			...
			// return error message
			return Response.status(400).entity(gson.toJson(error)).build();
		} else if (vm.getState() !=  educloud.internal.entities.VirtualMachine.VMState.DONE) {
	...
	public Response stopVM(String machine) {
		...
		LOG.debug("Application will stop a VM  (Server Side)");
		LOG.debug(machine);
		...
		if (vm == null) {
			...
			return Response.status(400).entity(gson.toJson(error)).build();
			 ...
			 educloud.internal.entities.VirtualMachine.VMState.RUNNING ){
	...
	public Response changeVM(String machine) {
		...
		/*
		LOG.debug("Carregada a VM DO BANCO => " + vm.getName());
		LOG.debug("Carregada a VM DO BANCO mem => " + vm.getMemorySize());
		LOG.debug("Carregada a VM DO BANCO proc => " + vm.getNumberProcessors());
		LOG.debug("Carregada a VM DO BANCO cap => " + vm.getCapProcessor());
		
		LOG.debug("Tenho de jogar para o banco os dados: ");
		
		LOG.debug("Carregada a VM DO POST => " + externalMachine.getName());
		LOG.debug("Carregada a VM DO POST mem => " + externalMachine.getMemorySize());
		LOG.debug("Carregada a VM DO POST proc => " + externalMachine.getNumberProcessors());
		LOG.debug("Carregada a VM DO POST cap => " + externalMachine.getCapProcessor());
		*/
		/* vm start logic */
		VMManager vmManager = new VMManager();
		
		vm.setMemorySize(externalMachine.getMemorySize());
		vm.setNumberProcessors(externalMachine.getNumberProcessors());
		vm.setCapProcessor(externalMachine.getCapProcessor());
		
		vm = vmManager.scheduleChangeVM(vm);
		externalMachine.setState(VMState.valueOf(vm.getState().name()));
	...
	public Response cloneVM(String machine) {
		...
		LOG.debug("Application will clone a new VM (Server Side)");
		
		VirtualMachine externalMachine = gson.fromJson(machine, VirtualMachine.class);
		
		 educloud.internal.entities.VirtualMachine clonelMachine = new  educloud.internal.entities.VirtualMachine();
		
		clonelMachine.setCapProcessor(externalMachine.getCapProcessor());
		clonelMachine.setDescription("Clonned VM FROM vm "+externalMachine.getId());
		clonelMachine.setMemorySize(externalMachine.getMemorySize());
		clonelMachine.setName("clone_"+externalMachine.getName());
		clonelMachine.setNumberProcessors(externalMachine.getNumberProcessors());
		clonelMachine.setOsType(externalMachine.getOsType());
		clonelMachine.setUserId(externalMachine.getUserId());
		
		clonelMachine.setMachineParent(externalMachine.getId());
		
		//Logica de inicializacao da VM.
		VMManager vmManager = new VMManager();
		vmManager.cloneVM(clonelMachine);
		
		LOG.debug("Clone sending!");
	...
	public Response describeInstances() {
		...
		List< educloud.internal.entities.VirtualMachine> listaVirtualMachines = null;
		...
		for(  educloud.internal.entities.VirtualMachine vmInterna : listaVirtualMachines )
		{
			...
			vmRetorno.setNumberProcessors(vmInterna.getNumberProcessors());
			vmRetorno.setCapProcessor(vmInterna.getCapProcessor());
			vmRetorno.setLoadBalancer(vmInterna.getLoadBalancer());
			vmRetorno.setIpLoadBalancer(vmInterna.getIpLoadBalancer());
			vmRetorno.setOsType(vmInterna.getOsType());
			vmRetorno.setLoadBalancer(vmInterna.getLoadBalancer());
			vmRetorno.setIpLoadBalancer(vmInterna.getIpLoadBalancer());
			
			System.out.println("AQUI => " + vmInterna.getLoadBalancer());
			
			if(vmInterna.getLoadBalancer() == null)
				vmRetorno.setWithLoadBalancer(false);
			else
				vmRetorno.setWithLoadBalancer(true);
	...
	public Response getVM(@PathParam("id") String vmId) {
		...
		int id = Integer.valueOf(vmId);
		 ...
		 educloud.internal.entities.VirtualMachine vm = VirtualMachineDao.getInstance().findById(id);
		...
		virtualMachine.setNumberProcessors(vm.getNumberProcessors());
	...
	public Response deleteVirtualMachine(String jsonVms) {
		...
		VMManager vmManager = new VMManager();
			...
			for (Integer vmId : vms) {
				 educloud.internal.entities.VirtualMachine vm = VirtualMachineDao.getInstance().findById(vmId);
				if (null != vm) {
					vmManager.scheduleRemoveVM(vm);
					/*
					if (!user.isAdmin()) {
