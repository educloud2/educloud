...
import  educloud.cloudserver.scheduler.tasks.CheckNodeTask;
import  educloud.cloudserver.scheduler.tasks.CloudTask;
	...
	public List<CloudTask> findPendingTasks() {
		...
		try {
			...
			if (rs.next()) {
				...
				} else if(type.equals("CHANGEVM")){
					task = new ChangeVmTask();
				}
				else {
