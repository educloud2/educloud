package  educloud.cloudserver.database.dao;
...
public class VirtualMachineDao extends AbstractDao {
	...
	public void insert(VirtualMachine machine, Template template) {
		...
		try {
			...
			if (rs.next()) {
				...
				ps = getConnection().prepareStatement("INSERT INTO MACHINE (MACHINE_ID, USER_ID, NAME, DESCRIPTION, OS_TYPE, STATE, MEMORY, PROCESSOR,  LOAD_BALANCER) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
				ps.setInt(1, key);
				...
				//Adicionado info de processador
				ps.setInt(8, template.getNumberProcessors());
				
				if(machine.isWithLoadBalancer()){
					ps.setString(9, "LB_"+machine.getName());
				} else
					ps.setString(9, null);
	...
	public void insert(VirtualMachine machine) {
		PreparedStatement ps = null;
		ResultSet rs = null;
		LOG.debug("NODE ID = " + machine.getNodeId());
		LOG.debug("USER ID = " + machine.getUserId());
		try {
			ps = getConnection().prepareStatement("values next value for seq_machine_id");
			rs = ps.executeQuery();
			int key;
			if (rs.next()) {
				...
				ps = getConnection().prepareStatement("INSERT INTO MACHINE (MACHINE_ID, USER_ID, NAME, DESCRIPTION, OS_TYPE, STATE, MEMORY, PROCESSOR, MACHINE_PARENT) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
				ps.setInt(1, key);
				ps.setInt(2, machine.getUserId());
				ps.setString(3, machine.getName());
				ps.setString(4, machine.getDescription());
				ps.setString(5, machine.getOsType());
				ps.setString(6, machine.getState().name());
				ps.setLong(7, machine.getMemorySize());
				ps.setInt(8, machine.getNumberProcessors());
				ps.setInt(9, machine.getMachineParent());
				ps.execute();
			...
			LOG.error(e);
			LOG.error("Erro de SQL = " + e.getErrorCode() + " - " + e.getSQLState());
			LOG.error("Erro de SQL = " + e.getStackTrace() + " - " + e.getSQLState());
			try {
				getConnection().rollback();
			} catch (SQLException e1) {
				LOG.error(e1);
	...
	public VirtualMachine findById(int id) {
		...
		try {
			...
			if (rs.next()) {
				...
				vm.setNumberProcessors(rs.getInt("processor"));
				vm.setCapProcessor(rs.getInt("cap_processor"));
				...
				vm.setLoadBalancer(rs.getString("load_balancer"));
				vm.setIpLoadBalancer(rs.getString("ip_lb"));
				vm.setMachineParent(Integer.valueOf(rs.getString("machine_parent")));
				return vm;
			}
		} catch (SQLException e) {
			LOG.error(e);
	...
	public List<VirtualMachine> getAll() {
		...
		try {
			...
			while (rs.next()) {
				...
				vm.setMachineParent(rs.getInt("machine_parent"));
				vm.setLoadBalancer(rs.getString("load_balancer"));
				vm.setIpLoadBalancer(rs.getString("ip_lb"));
				machines.add(vm);
			}
	...
	public void update(VirtualMachine machine) {
		PreparedStatement ps = null;
		ResultSet rs = null;

		try {
			ps = getConnection().
					prepareStatement("UPDATE MACHINE SET MEMORY=?, PROCESSOR=?, CAP_PROCESSOR=? WHERE MACHINE_ID=?");
			ps.setInt(1, (int) machine.getMemorySize());
			ps.setInt(2, (int) machine.getNumberProcessors());
			ps.setInt(3, (int) machine.getCapProcessor());
	...
	public void updateParent(VirtualMachine machine) {
		PreparedStatement ps = null;
		ResultSet rs = null;

		try {
			ps = getConnection().
					prepareStatement("UPDATE MACHINE SET MACHINE_PARENT=? WHERE MACHINE_ID=?");
			ps.setInt(1, (int) machine.getMachineParent());
