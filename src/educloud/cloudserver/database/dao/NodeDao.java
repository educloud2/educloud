...
public class NodeDao extends AbstractDao {
	...
	public Node findNodeById(int nodeId){
		...
		try {
			...
			if (rs.next()) {
				...
				node.setAvailMemory(rs.getLong("avail_memory"));
				node.setTotalMemory(rs.getLong("total_memory"));
				return node;
			}
		} catch (SQLException e) {
			LOG.debug(e);
	...
	public void updateLastPing(Node node) {
		...
		try {
			ps = getConnection().prepareStatement("UPDATE NODE SET LAST_PING=?, VBOX_VERSION=?, VBOX_CONNECTED=?, AVAIL_MEMORY=?, TOTAL_MEMORY=? WHERE NODE_ID = ?");
			...
			ps.setLong(4, node.getAvailMemory());
			ps.setLong(5, node.getTotalMemory());
			ps.setInt(6, node.getId());
			ps.execute();
			getConnection().commit();
		} catch (SQLException e) {
			LOG.debug(e);
