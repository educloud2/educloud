	...
	public void insert(Template template) {
		...
		try {
			...
			if (rs.next()) {
				...
				ps = getConnection().prepareStatement("INSERT INTO TEMPLATE (TEMPLATE_ID, OS_TYPE, NAME, FILENAME, DESCRIPTION, MEMORY, PROCESSOR, DEVICE, STORAGE_PORT) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
				ps.setInt(1, key);
				...
				ps.setInt(7, template.getNumberProcessors());
				ps.setString(8, template.getDevice());
				ps.setInt(9, template.getPort());
				ps.execute();
	...
	public Template findById(int id) {
		...
		try {
			...
			if (rs.next()) {
				...
				tpl.setNumberProcessors(rs.getInt("processor"));
				tpl.setDevice(rs.getString("device"));
				tpl.setPort(rs.getInt("storage_port"));
				return tpl;
			}
		} catch (SQLException e) {
			LOG.error(e);
